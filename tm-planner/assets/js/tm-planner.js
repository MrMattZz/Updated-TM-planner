...
function createBooster(booster, s) {
    ...
    } else if($('#booster_' + b.id).hasClass('rec-char-alt')) {
...
function init(tmId, server) {
    ...
    var opponents = tm_opponents[tmId];
    var bossInfo = tm_boss_info[tmId];
    var bird = hasAmbush(tmId, server) ? 1.5 : 1.0;
...
function hasAmbush(tmId, server) {
    return tmId.toString().includes('2109') || tmId.toString().includes('2261') || tmId.toString().includes('2299') || tmId.toString().includes('2299')
            || tmId.toString().includes('2336') || tmId.toString().includes('2387') || (tmId.toString().includes('2443') && server.toString().includes('glb')) || tmId.toString().includes('2469') || tmId.toString().includes('2557')
            || tmId.toString().includes('2659') || tmId.toString().includes('2690'); 
}
...
$(document).ready(function() {
    ...
    $('.team-slot, .ambush-team-slot').droppable({
        ...
        drop: function(event, ui) {
            ...
            var server = $('#tm-select').val().substring($('#tm-select').val().indexOf('_') + 1).trim();
            if(!$(ui.draggable).hasClass('booster-ambush-clone') && previousTeam === -1 && !inDupeBoosters && hasAmbush($('#tm-select').val(), server)) {
